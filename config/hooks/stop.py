...
def report_stale_domains():
    """Read .touched_domains, print self-improve commands, clean up."""
    project = os.environ.get("CLAUDE_PROJECT_DIR", str(Path(__file__).resolve().parent.parent.parent))
    ...
    if not raw:
        ...
        return
    domains = sorted(set(raw.splitlines()))
    print("\n\u26a0  Codebase changed \u2014 sync expertise:")
    for d in domains:
        print(f"   /mental-model:{d}:self-improve")
    sentinel.unlink(missing_ok=True)
...
def main():
    try:
        ...
        # infinite loops.  This means Claude is already continuing from a
        # previous stop hook decision.
        if stop_hook_active:
            sys.exit(0)

        report_stale_domains()

        # Logging handled by log_event.py (JSONL)
        ...
        if args.chat and 'transcript_path' in input_data:
            ...
            if os.path.exists(transcript_path):
                ...
                try:
                    ...
                    chat_dir = os.path.join(os.getcwd(), log_subdir, session_id)
