---
name: reviewer-npeltier-plan
allowed-tools: Agent
description: Load npeltier's review values as constraints before planning a PR. Spawns a Plan subagent that ensures the plan avoids all known red flags. Returns ~300 tokens.
argument-hint: [what you want to implement]
---

# Reviewer Mental Model: npeltier — Plan

Spawn a **Plan subagent** with this prompt:

> You are planning a PR that will be reviewed by npeltier (Nicolas Peltier), tech lead of mas/io.
>
> **Step 1**: Read `.claude/commands/mental-model/reviewer-npeltier/expertise.yaml`.
> Extract:
> - His top 3 values (scope_discipline, factorization_and_reuse, no_dead_or_throwaway_code)
> - All blocking red_flags
> - Relevant domain_opinions for the areas this change touches
> - His ownership_map to understand review depth per directory
>
...
> - **PR size estimate**: Small/medium/large — npeltier prefers small, focused PRs
